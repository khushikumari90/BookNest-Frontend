...
@Component({
        ...
        <div class="info-col">
          ...
          <div class="actions" *ngIf="book.stock > 0">
            <div class="qty-row">
              <label>Qty:</label>
              <select [(ngModel)]="qty">
                <option *ngFor="let n of [1,2,3,4,5]">{{n}}</option>
              </select>
            </div>
        ...
        <div class="review-list">
          <div class="review-card" *ngFor="let r of reviews">
            <div class="review-header">
              <div class="reviewer-avatar">U{{r.userId}}</div>
              <div>
                <div class="stars">
  ...
  styles: [`
    ...
    .reviewer-avatar { width: 36px; height: 36px; border-radius: 50%; background: var(--primary); color: white; font-size: 12px; display: flex; align-items: center; justify-content: center; font-weight: 700; }
...
export class BookDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.reviewSvc.getByBook(id).subscribe({ next: r => this.reviews = r, error: () => {} });
  ...
  submitReview() {
    ...
    this.reviewSvc.addReview({ bookId: this.book!.bookId, userId: uid, rating: this.review.rating, comment: this.review.comment, verified: true }).subscribe({
      next: r => { this.reviews.unshift(r); this.review = { rating: 0, comment: '' }; },
      error: () => {}
    });
  }
