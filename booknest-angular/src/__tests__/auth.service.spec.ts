...
describe('AuthService', () => {
  ...
  it('should return true for isLoggedIn when user stored in localStorage', () => {
    ...
    // Re-create service so BehaviorSubject picks up stored value
    service = new (AuthService as any)(TestBed.inject(require('@angular/common/http').HttpClient));
  ...
  it('should POST to /api/auth/login and store token in localStorage', () => {
    ...
    // wallet creation triggered inside nested subscribe
    const walletHttp = httpMock.expectOne(`/api/wallet/create/${mockUser.userId}`);
    walletHttp.flush({});
  });
  ...
  it('should throw error if admin secret is wrong', () => {
    expect(() =>
      service.registerAdmin({ fullName: 'Admin', email: 'a@b.com', password: 'p', mobile: '0', adminSecret: 'wrong' })
    ).toThrow('Invalid admin secret key.');
  });

  it('should POST /api/auth/register/admin with correct secret', () => {
    const req = { fullName: 'Admin', email: 'admin@b.com', password: 'p', mobile: '0', adminSecret: 'booknest@admin2026' };
